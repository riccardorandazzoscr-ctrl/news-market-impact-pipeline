...
Automazione: task locali nell'app ChatGPT (routine 07:30, KB 07:55, market data
08:00, analisi 08:15 con retry 09:15/10:15, scorecard lunedì 09:00, mensile il 1°
alle 09:30; controllo alle 11:30). I 5 vecchi job launchd sono disabilitati.
Configurazione, modelli e verifica: `references/chatgpt_routine.md`.
...
- ⚠ **Le research le scrive il maintainer, non Codex** (2026-08-16, precisato 2026-08-18).
  Codex non scrive deep research né i relativi prompt di propria iniziativa: davanti a una
  ...
  Codex riprende il file solo per indicizzarlo e cancellare il prompt.
- ⚠ **Nessuna API key Anthropic**: il classificatore è Codex autenticato con ChatGPT;
  Python espone solo tool deterministici via Bash. `anthropic` non si usa.
- ⚠ `update_market_data.py` importa costanti e helper da `bootstrap_market_data.py` —
  **non separarli**.
- ⚠ I task ChatGPT usano percorsi assoluti su questo Mac: se la cartella si sposta,
  aggiorna i prompt delle attività programmate prima del prossimo run.
